...
def getDictFormattedDatabases(listDatabases):
    #Create the dictionary and list of languages with dictionary
    dictionary={}
    language=()
    FormattedDict=[]
    
    for line in listDatabases:
        language=line[0]
        dictionary=dict(line[1])
        FormattedDict.append((language,dictionary))
...
def translateFromDict(text, dictBasedDb):
    ...
    subsequent word.
    """  
    #Figure out translations of words regardless of punctuation    
    translation = ""
    words = text.split()
    for word in words:
        toAppend = ''
...
def performDictBasedTranslations(preprocessedText, dictBasedDatabases):
    ...
    """
    #Display the proper translations with the correct languages
